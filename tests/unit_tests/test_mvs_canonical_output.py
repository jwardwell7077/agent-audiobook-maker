"""Integration tests for MVS canonical output and purge regression.

Tests ingesting a real sample PDF, volume structure, and purge regression logic.
"""
...
def test_mvs_canonical_volume_and_intro(monkeypatch, tmp_path):
    ...
    source_pdf = Path("data/books/MVS/source_pdfs") / PDF_NAME
    if not source_pdf.exists():  # skip if canonical pdf missing
        import pytest

        pytest.skip("canonical MVS pdf missing from repo")
    ...
    expected_vol = json.loads((EXPECTED_DIR / "real_sample_volume.json").read_text(encoding="utf-8"))
    ...
    expected_intro = json.loads((EXPECTED_DIR / "00000.json").read_text(encoding="utf-8"))
...
def test_mvs_purge_regression(monkeypatch, tmp_path):
    ...
    r1 = client.post("/ingest", data={"book_id": BOOK_ID, "pdf_name": PDF_NAME})
    ...
    r2 = client.post("/ingest", data={"book_id": BOOK_ID, "pdf_name": PDF_NAME})
    ...
    for ch_id, expected_hash in expected_hashes.items():
        ...
        assert p.exists(), f"chapter json missing after purge reingest: {ch_id}"
        ...
        if got != expected_hash:
            ...
            raise AssertionError(
                f"hash mismatch {ch_id} expected={expected_hash} got={got}\n" + "text preview:\n" + preview
            )
