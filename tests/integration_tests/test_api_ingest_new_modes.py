...
REAL_PDF_SRC = (
    _P(__file__).resolve().parent.parent / "test_data" / "real_sample.pdf"
)
...
async def test_ingest_single_stored_pdf(tmp_path: Path, monkeypatch):
    # isolate cwd
    old = Path.cwd()
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            r = await client.post(
                "/ingest", data={"book_id": "bk1", "pdf_name": "chunks1.pdf"}
            )
...
async def test_ingest_batch_mode(tmp_path: Path, monkeypatch):
    old = Path.cwd()
    monkeypatch.chdir(tmp_path)
    try:
