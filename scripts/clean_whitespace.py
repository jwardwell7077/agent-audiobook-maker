...
def process_file(file_path: Path, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        if lines_cleaned > 0:
            if dry_run:
                print(
                    f"[DRY RUN] Would clean {lines_cleaned} empty lines "
                    f"in: {file_path}"
                )
                return True
            else:
...
def find_python_files(path: Path) -> list[Path]:
    ...
    if path.is_file() and path.suffix == '.py':
        ...
        python_files.extend([
            py_file for py_file in path.rglob('*.py')
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Clean empty lines in Python files by removing spaces/tabs "
...
Examples:
  ...
  %(prog)s --verbose src/             # Show detailed output for all files
    ...
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
        help='Show output for all files, including those with no changes'
    )
