# /home/jon/repos/audio-book-maker-lg/lf_components/loaders/chapter_volume_loader.py
...
from langflow.io import StrInput, IntInput, Output, BoolInput
...
class ChapterVolumeLoader(Component):
    ...
    description = (
        "Load an existing volume manifest and selected chapter from disk."
    )
    icon = "book"
    ...
    def _resolve_manifest_path(self) -> Path:
        ...
        if self.directory:
            ...
            if not candidates:
                raise FileNotFoundError(
                    "No '*_volume.json' found in "
                    f"{d} (or subfolders). Provide a Manifest Path or"
                    " Directory."
                )
            chosen = candidates[-1]
            if self.log_debug:
        ...
        raise ValueError(
            "Provide either Manifest Path or Directory to locate the manifest."
        )
    ...
    def _resolve_chapter_path(
        self, json_path_value: str, manifest_dir: Path
    ) -> Path:
    ...
    def load_chapters_index(self):  # returns plain list
    ...
    def load_selected_chapter(self):  # returns plain dict
        manifest = self._load_manifest()
        ...
        if idx < 0 or idx >= len(chapters):
            raise ValueError(
                f"Chapter index {idx} out of range (0..{len(chapters) - 1})."
            )
        ...
        chapter_path = self._resolve_chapter_path(
            json_path_value, manifest_dir
        )
