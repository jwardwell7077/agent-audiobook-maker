...
class ABMSpeakerAttribution(Component):
    ...
    def __init__(self):
        ...
        self.speech_verbs = [
            'said', 'replied', 'asked', 'shouted', 'whispered', 'called',
            'announced', 'declared', 'exclaimed', 'muttered', 'gasped',
            'stammered', 'yelled', 'screamed', 'chuckled', 'laughed'
        ]
    ...
    def attribute_speaker(self) -> Data:
        ...
        try:
            ...
            chapter_characters = self._load_chapter_characters(
                utterance_data.get('book_id', ''),
                utterance_data.get('chapter_id', '')
            )
    ...
    def _analyze_attribution(self, utterance_data: Dict[str, Any],
                            chapter_characters: Dict[str, CharacterProfile]) -> AttributionResult:
        """Attribution Analysis - Multi-Method Speaker Detection"""
    ...
    def _contextual_attribution(self, dialogue_text: str, context_before: str, 
                              context_after: str) -> Optional[AttributionResult]:
    ...
    def _manage_character(self, attribution_result: AttributionResult, 
                         utterance_data: Dict[str, Any]) -> Optional[CharacterProfile]:
    ...
    def _build_character_profile(self, character_profile: CharacterProfile, 
    ...
    def _generate_output(self, attribution_result: AttributionResult,
                        character_profile: Optional[CharacterProfile],
                        utterance_data: Dict[str, Any]) -> Dict[str, Any]:
        """Generate structured output"""
        ...
        if attribution_result.character_id:
            self.conversation_history.append((
                attribution_result.character_id, 
                utterance_data["dialogue_text"]
            ))
            # Keep recent history
    ...
    def _clean_character_name(self, name: str) -> str:
        ...
        stopwords = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'he', 'she', 'it', 'they'}
        words = name.split()
    ...
    def _create_new_character(self, attribution_result: AttributionResult, 
                            utterance_data: Dict[str, Any]) -> CharacterProfile:
    ...
    def _update_character(self, character_profile: CharacterProfile,
                         attribution_result: AttributionResult,
                         utterance_data: Dict[str, Any]) -> CharacterProfile:
        """Update existing character profile"""
        
        # Update last appearance
        character_profile.last_appearance = f"{utterance_data.get('book_id', '')}_{utterance_data.get('chapter_id', '')}"
