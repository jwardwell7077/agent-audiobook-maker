"""ABM Chunk Iterator for LangFlow Two-Agent Processing."""
...
class ABMChunkIterator(Component):
    ...
    description = "Process chunks through two-agent pipeline with batch management"
    icon = "repeat"
    name = "ABMChunkIterator"
    ...
    inputs = [
        ...
        IntInput(
            ...
            info="Number of chunks to process per batch",
            value=10,
            required=False,
        ),
        IntInput(
            name="start_chunk",
            display_name="Start Chunk ID",
            info="Start processing from this chunk (for debugging)",
            value=1,
            required=False,
        ),
        IntInput(
            ...
            info="Limit processing to this many chunks (0 = all)",
            value=0,
            required=False,
        ),
        BoolInput(
            name="dialogue_priority",
            display_name="Prioritize Dialogue Chunks",
            info="Process dialogue chunks first",
            value=True,
            required=False,
        ),
    ]
    ...
    def get_next_utterance(self) -> Data:
        """Get the next chunk formatted for two-agent processing."""
        try:
            ...
            if not chunks:
                self.status = "No chunks to process"
                return Data(data={"error": "No chunks available"})
            ...
            # Update status
            self.status = (
                f"Processing chunk {current_chunk['chunk_id']}/{len(filtered_chunks)} - {current_chunk['type']}"
            )
    ...
    def _filter_and_sort_chunks(self, chunks: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Filter and sort chunks based on processing preferences."""
        filtered_chunks = chunks.copy()

        # Filter by start chunk
    ...
    def _prepare_utterance_for_agents(self, chunk: dict[str, Any], chapter_data: dict[str, Any]) -> dict[str, Any]:
        """Prepare chunk data for two-agent processing pipeline."""
        ...
        utterance_data = {
            ...
            "pipeline_info": {
                "source_component": "ABMChunkIterator",
    ...
    def _create_completion_summary(self, chapter_data: dict[str, Any]) -> Data:
        """Create summary when all chunks are processed."""
        ...
        self.status = f"Completed processing {self._total_processed} chunks"
